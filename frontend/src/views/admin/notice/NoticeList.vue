...
import { getAdminNoticeList, createNotice, updateNotice, deleteNotice } from '@/api/notice'
...
function handleEditorCreated(editor) { editorRef.value = editor }
...
function openDialog(row) {
  ...
  Object.assign(form, {
    title: row?.title || '', content: row?.content || '', status: row?.status ?? 1
  })
