  ...
  <div class="page-container">
    <div class="page-header">
      ...
      <div class="header-actions">
        <el-input v-model="keyword" placeholder="搜索姓名/用户名" clearable style="width: 240px" @keyup.enter="loadList">
...
import { getAdminDoctorList, createDoctor, updateDoctor, deleteDoctor, changeDoctorStatus } from '@/api/doctor'
...
const rules = {
  ...
  confirm_password: [{ validator: validateConfirmPassword(() => form.password), trigger: 'blur' }],
...
async function loadList() {
  loading.value = true
  try {
    const data = await getAdminDoctorList({
      page: page.value,
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEdit.value) {
      await updateDoctor(editId.value, form)
...
async function toggleStatus(row) {
  ...
  try {
    await changeDoctorStatus(row.id, newStatus)
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm(`确定删除医生「${row.real_name}」吗？`, '提示', {
